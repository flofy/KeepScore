...
function GameScreen({ initialGame, onNewGame, onSavedGames }: { initialGame: Game; onNewGame: () => void; onSavedGames: () => void }) {
  ...
  return <main className={fullscreen ? 'app-shell fullscreen' : 'app-shell'}>
    <header className="app-header game-header"><div><p className="eyebrow">SCORE KEEPER</p><h1>{game.name || t('appName')}</h1></div><div className="toolbar"><InstallButton/><button className="secondary-button" type="button" onClick={onSavedGames}>{t('savedGames')}</button><button className="secondary-button" type="button" onClick={onNewGame}>{t('newGame')}</button></div></header>
