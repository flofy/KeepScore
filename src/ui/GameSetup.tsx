...
export function GameSetup({
  onCreate,
  onBack,
  onDetermineStartingPlayer,
}: Props) {
  const { t } = useI18n();
  const [names, setNames] = useState(["Alice", "Bob"]);
  const [colors, setColors] = useState<string[]>(() =>
    names.map((_, index) => colorForIndex(index)),
  );
  ...
  function setColor(index: number, color: string) {
    setColors((current) =>
      current.map((current_color, i) => (i === index ? color : current_color)),
    );
  }
  ...
  return (
    <main className="setup">
      <div className="setup-card">
        ...
        <section className="setup-section">
          <label className="field-label" htmlFor="starting-score">
            <span>{t("startingScore")}</span>
            <span className="optional">{t("startingScoreHint")}</span>
          </label>
          <input
            id="starting-score"
            className="setup-input"
            ...
            {GAME_PRESETS.map((preset) => (
              ...
              >
                ...
                <span className="preset-name">
                  {preset.id === "custom" ? t("presetCustom") : preset.name}
                </span>
        ...
        <section className="setup-section">
          <div className="section-heading">
            <div>
              <p className="field-kicker">{t("players")}</p>
              <h2>{t("whoIsPlaying")}</h2>
            </div>
            <span className="player-count">{names.length}</span>
          </div>
            ...
            {names.map((name, index) => (
              <div className="player-editor" key={index}>
                ...
                <div
                  className="color-row"
                  role="radiogroup"
                  aria-label={t("colorForPlayer") + " " + (index + 1)}
                >
