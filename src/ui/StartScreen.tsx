    ...
    <main className="start-screen">
      <div className="start-card">
        ...
        <div className="start-options">
          <button type="button" className="start-option-btn primary" onClick={onNewGame}>
            <span className="option-icon">🎮</span>
            <span className="option-label">{t('newGame')}</span>
            <span className="option-desc">{t('startNewGame')}</span>
          </button>

          <div className="option-divider"><span>{t('or')}</span></div>

          <div className="chwatzi-option">
            <button type="button" className="start-option-btn primary chwatzi-btn" onClick={handleChwatzi} disabled={playerCount < 2}>
              <span className="option-icon">🎲</span>
            ...
            <div className="player-count-selector">
              <button type="button" className="count-btn" onClick={handleDecrement} disabled={playerCount <= 2} aria-label="Decrease players">−</button>
              <span className="count-value">{playerCount}</span>
              <input type="range" min={2} max={10} value={playerCount} onChange={handlePlayerCountChange} className="count-slider" aria-label="Number of players" />
              <button type="button" className="count-btn" onClick={handleIncrement} disabled={playerCount >= 10} aria-label="Increase players">+</button>
            </div>
          </div>
        </div>
      </div>
