...
type ControlPointer = { pointerId: number; offsetX: number; offsetY: number; startX: number; startY: number; startedAt: number };
...
export function GameScreen({
  ...
  const { fullscreen, toggleFullscreen, keepHeaderInFullscreen, setKeepHeaderPreference } = useFullscreen();
  const { editingEntry, draftDelta, historyOpen, historyFlipped, historyGrouping, setHistoryGrouping, setHistoryFlipped, setDraftDelta, beginEdit, saveEdit, openHistory: openHistoryView, closeHistory, cancelEdit, deleteEntry } = useGameHistoryView({ game, dispatch });
  const { swapped, orderedPlayers, playerRotations, toggleSwap, togglePlayerRotation } = useGamePlayerView(game);
  ...
  const handleControlPointerDown = (event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    dragRef.current = { pointerId: event.pointerId, offsetX: event.clientX - controlPosition.x, offsetY: event.clientY - controlPosition.y, startX: event.clientX, startY: event.clientY, startedAt: performance.now() };
  };

  const handleControlPointerMove = (event: React.PointerEvent<HTMLButtonElement>) => {
    const drag = dragRef.current;
    if (!drag || drag.pointerId !== event.pointerId) return;
    setControlPosition(clampControlPosition({ x: event.clientX - drag.offsetX, y: event.clientY - drag.offsetY }));
  };
  ...
  const handleControlPointerUp = (event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    if (elapsed <= TAP_MAX_DURATION_MS && distance <= TAP_MAX_DISTANCE_PX) toggleFullscreen();
  };

  const handleControlPointerCancel = (event: React.PointerEvent<HTMLButtonElement>) => {
  ...
  const addScore = (playerId: string, delta: number) => { dispatch({ type: "ADD_SCORE", playerId, delta }); haptic(); };
  ...
  const updateMunchkinStats = (playerId: string, stats: MunchkinStats) => { dispatch({ type: "UPDATE_MUNCHKIN_STATS", playerId, stats }); haptic(); };
  ...
  const addPlayer = () => { dispatch({ type: "ADD_PLAYER" }); appToast.success(t("playerAdded")); haptic(); };
  ...
  const shellClassName = ["app-shell", fullscreen ? "fullscreen" : "", fullscreen && !keepHeaderInFullscreen ? "fullscreen-hide-header" : "", game.presetId === "munchkin" ? "munchkin-theme" : ""].filter(Boolean).join(" ");
  ...
  return (
    <>
      <main className={shellClassName}>
        ...
        {!keepHeaderInFullscreen && fullscreen && <button className="fullscreen-exit-control" type="button" onPointerDown={handleControlPointerDown} onPointerMove={handleControlPointerMove} onPointerUp={handleControlPointerUp} onPointerCancel={handleControlPointerCancel} style={{ left: controlPosition.x, top: controlPosition.y }} aria-label={t("exitFullscreen")}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 4v5H4M15 4v5h5M9 20v-5H4M15 20v-5h5" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" fill="none" /></svg></button>}
