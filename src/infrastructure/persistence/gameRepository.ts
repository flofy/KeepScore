...
export interface GameRepository { list(): Game[]; get(id: string): Game | undefined; save(game: Game): void; remove(id: string): void }

function readGames(): Game[] {
  if (typeof localStorage === 'undefined') return []
  try { const raw = localStorage.getItem(STORAGE_KEY); return raw ? JSON.parse(raw) as Game[] : [] } catch { return [] }
}
function writeGames(games: Game[]) { localStorage.setItem(STORAGE_KEY, JSON.stringify(games)) }
