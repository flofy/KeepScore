...
export function gameReducer(game: Game, action: GameAction): Game {
  ...
  switch (action.type) {
    case "ADD_SCORE": {
      ...
      if (!player || !Number.isFinite(action.delta) || action.delta === 0)
        return game;
      return {
        ...game,
    ...
    case "REMOVE_PLAYER": {
      ...
      return {
        ...
        history: game.history.filter(
          (entry) => entry.playerId !== action.playerId,
        ),
        updatedAt,
      };
    }
    case "EDIT_HISTORY_ENTRY":
      return updateHistoryEntry(game, action.entryId, action.delta);
    case "DELETE_HISTORY_ENTRY":
      return removeHistoryEntry(game, action.entryId);
  }
}
