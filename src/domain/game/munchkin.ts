...
export function addCombatHelper(
  ...
  return {
    ...workflow,
    context: { ...workflow.context, combat: { ...combat, helperIds: [...combat.helperIds, playerId] } },
  }
}
...
export function resolveCombat(
...
): { workflow: MunchkinWorkflow; winner: boolean; totalPower: number } {
  const combat = workflow.context.combat
  ...
  const winner = totalPower > combat.monsterLevel
  return {
    workflow: {
      ...workflow,
      phase: 'turn',
