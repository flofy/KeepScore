...
async function startGame(page: Parameters<typeof test>[0]["page"]) {
  await page.goto("setup", { waitUntil: "networkidle" });
  await page.getByRole("button", { name: "Start game" }).click();
  await expect(page.locator(".players")).toBeVisible();
}

async function openHistory(page: Parameters<typeof test>[0]["page"]) {
...
test.describe("history grouping", () => {
  test("switches grouping and preserves the matching history entries", async ({
    ...
    await playerCards.nth(0).getByRole("button", { name: /Add one point to/ }).click();
    await playerCards.nth(1).getByRole("button", { name: /Add one point to/ }).click();
    ...
    await expect(dialog.getByRole("heading", { name: "Round 1" })).toBeVisible();
